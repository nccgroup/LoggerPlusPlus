...
public class TableHeaderMenu extends JPopupMenu{
						...
						public void run() {
							...
							if(!newValue.equals(columnObj.getRegExData().getRegExString())){
								...
								try {
						            ...
						            BurpExtender.getCallbacks().printError("provided regular expression was wrong. It cannot be saved.");
	...
	public void saveAndReloadTableSettings(){
		...
		if(BurpExtender.getLoggerInstance().getLoggerPreferences().getAutoSave()){
			BurpExtender.getLoggerInstance().getLoggerPreferences().setAutoSave(false);
			BurpExtender.getLoggerInstance().getLoggerOptionsPanel().getFileLogger().setAutoSave(false);
