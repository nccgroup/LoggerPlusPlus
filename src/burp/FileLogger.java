...
public class FileLogger implements LogEntryListener{
    ...
    FileLogger(){
        loggerPreferences = BurpExtender.getInstance().getLoggerPreferences();
    ...
    public void setAutoSave(boolean enabled) {
        if (enabled) {
            ...
            if (autoSaveFile != null) {
                ...
                try {
                    ...
                    BurpExtender.getInstance().addLogListener(this);
        ...
        BurpExtender.getInstance().getLoggerOptionsPanel().setAutoSaveBtn(enabled);
    }

    @Override
    public void onRequestReceived(final LogEntry logEntry) {

    }

    @Override
    public void onResponseReceived(final LogEntry logEntry) {
        Thread saveThread = new Thread(){
            @Override
            public void run() {
                synchronized (autoSaveWriter){
                    autoLogItem(logEntry);
                }
            }
        };
        saveThread.start();
