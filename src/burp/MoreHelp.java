...
public class MoreHelp {
	...
	public static void checkForUpdate(boolean showMessages) {
		IExtensionHelpers helper = BurpExtender.getInstance().getCallbacks().getHelpers();
		...
		try{
			...
			byte[] response = BurpExtender.getInstance().getCallbacks().makeHttpRequest(changeLogURL.getHost(), 443, true, request);
		...
		switch(updateStatus){
			...
			case 1:
				...
				if(BurpExtender.getInstance().getCallbacks().isExtensionBapp()){
					updateMessage += "\nAs you are using BApp Store, you have to remove it first and download the Jar file from the GitHub repository. ";
				}else{
					if(BurpExtender.getInstance().getCallbacks().getExtensionFilename() != null){
						...
						if(res == JOptionPane.OK_OPTION){
							try {
								...
								Path outputPath = Paths.get(BurpExtender.getInstance().getCallbacks().getExtensionFilename());
							...
							BurpExtender.getInstance().getCallbacks().unloadExtension();
							return;
						}
					}
				}
