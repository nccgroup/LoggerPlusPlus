...
public class SanityCheckVisitor implements FilterParserVisitor{
  ...
  public VisitorData visit(ASTComparison node, VisitorData visitorData){
    ...
    if(node.booleanOperator == BooleanOperator.LESS_THAN || node.booleanOperator == BooleanOperator.LESS_THAN_EQUAL
            || node.booleanOperator == BooleanOperator.GREATER_THAN || node.booleanOperator == BooleanOperator.GREATER_THAN_EQUAL) {
      ...
      if (!valid) {
        visitorData.addError(String.format("Operator %s cannot be used to compare groups of type %s and %s.",
                node.booleanOperator, leftType.getTypeName(), rightType.getTypeName()));
      }
    ...
    }else if(node.booleanOperator == BooleanOperator.CONTAINS && !(String.class.isAssignableFrom(leftType) && String.class.isAssignableFrom(leftType))){
